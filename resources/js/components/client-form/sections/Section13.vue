<template>
    <div class="container">
        <content-box title="Section 12 - Pensions">
            <p class="text-gray-500 m-0">If the deceased received, or had made provision for, a pension or benefit from
                an employer or under a personal pension policy (other than the State Pension) you are required to submit
                details to HMRC.</p>
        </content-box>
        ...
        <content-box title="12.1 Pension Details">
            <yes-no collapse :label="formData[0].query" v-model="formData[0].answer">
                <div class="mt-4"></div>
                <base-input label="Name of pension scheme or title of pension policy"
                    placeholder="Personal pension programme" v-model="formData[0].onTrue.name_of_pension"></base-input>

                <label>Please describe the pension</label>
                <textarea v-model="formData[0].onTrue.description" class="form-control mb-4" rows="4"
                ...
                <base-input label="Company name" placeholder="Appleseed Holdings"
                    v-model="formData[0].onTrue.company_name"></base-input>
                <base-input label="Forename" placeholder="John" v-model="formData[0].onTrue.forename"></base-input>
                <base-input label="Surname" placeholder="Doe" v-model="formData[0].onTrue.surname"></base-input>
        ...
        <content-box title="12.2 Pension Payment">
            <yes-no collapse :label="formData[1].query" v-model="formData[1].answer">
                <div class="mt-4"></div>
                <base-input label="Name of pension scheme or title of pension policy"
                    placeholder="Personal pension programme" v-model="formData[1].onTrue.name_of_pension"></base-input>

                <label>Please describe the pension</label>
                <textarea v-model="formData[1].onTrue.description" class="form-control mb-4" rows="4"
                    placeholder="Please include a full overview of relevant details to this question"></textarea>

                <base-input label="What is the value of the lump sum?" placeholder="£300,000" type="number" min="0"
        ...
        <content-box title="12.3 Pension Payment">
            <yes-no collapse :label="formData[2].query" v-model="formData[2].answer">
                <div class="mt-4"></div>
                <base-input label="Name of pension scheme or title of pension policy"
                    placeholder="Personal pension programme" v-model="formData[2].onTrue.name_of_pension"></base-input>

                <label>Please describe the pension</label>
                <textarea v-model="formData[2].onTrue.description" class="form-control mb-4" rows="4"
                    placeholder="Please include a full overview of relevant details to this question"></textarea>
            </yes-no>
        </content-box>
