    ...
    <div class="container">
        <content-box title="Section 11 - Trusts in which the deceased had an interest">
            <yes-no collapse :label="formData[0].query" v-model="formData[0].answer">
                ...
                <base-input label="Surname" placeholder="Doe" v-model="formData[0].onTrue.trust_surname"></base-input>
                ...
                <textarea v-model="formData[0].onTrue.nature_of_inheritance" class="form-control mb-4" rows="4"
                ...
                <div class="row">
                    <div class="col-md-4 form-group">
                        <base-input type="number" placeholder="£240,000" min="0"
                ...
                <textarea v-model="formData[0].onTrue.trustees" class="form-control mb-4" rows="4"
                    placeholder="John Doe, Johnny Appleseed"></textarea>

                <label>Please add the contact details of one of the following</label>
                <switch-with-text left-text="Their Solicitors" right-text="Their accountants" class="form-group"
                    v-model="formData[0].onTrue.contact" return-text></switch-with-text>

                <honorific v-model="formData[0].onTrue.contact_honorific" />
                <base-input label="Forename" placeholder="John"
                    v-model="formData[0].onTrue.contact_forename"></base-input>
                <base-input label="Surname" placeholder="Doe" v-model="formData[0].onTrue.contact_surname"></base-input>

                <div class="row">
                    <div class="col-12 col-lg-6">
                        <base-input label="Phone number" placeholder="+44 012345 67890"
                            v-model="formData[0].onTrue.contact_phone"></base-input>
                    </div>
                    <div class="col-12 col-lg-6">
                        <base-input label="Email Address" placeholder="John.doe@doe.co.uk"
                            v-model="formData[0].onTrue.contact_email"></base-input>
                    </div>
                </div>

                <label>If you have a copy of the trust documents please include details below and upload the relevant
                    documents too</label>
                <div v-for="(doc, index) in formData[0].onTrue.docs" v-show="activeDoc === index" :key="index">
                    <div class="row no-gutters mt-4 form-group" :key="index">
                        <div class="col-md-4 mr-md-2 mb-2 mb-md-0">
                            <base-input placeholder="Document title e.g ‘The Will’" :form-group="false"
                    ...
                    <textarea v-model="doc.extra_details" :key="`details${index}`" class="form-control mb-4" rows="4"
                ...
                <div class="d-sm-flex align-items-center mb-4">
                    <div class="flex-grow-1 mb-3" v-show="formData[0].onTrue.docs.length > 1">
                        ...
                        <a href="#" v-if="formData[0].onTrue.docs.length > 1"
                            @click.prevent="removeDoc(activeDoc)">Remove document</a>
                    </div>
                    <base-button v-if="activeDoc > 0" type="default" outline @click="activeDoc--">Back</base-button>
                    <base-button type="default" outline @click="addDoc"
                        v-if="activeDoc + 1 == formData[0].onTrue.docs.length">Add</base-button>
                    <base-button type="default" outline @click="activeDoc++"
                        v-if="activeDoc + 1 < formData[0].onTrue.docs.length">Next</base-button>
                </div>

                <a v-b-toggle.collapse1 class="pointer">Tip<i class="icon-xs fas fa-chevron-down ml-2"></i></a>
                <b-collapse visible id="collapse1">
                    <p class="text-gray-500 mt-2 mb-0">Domicile and residence is relevant because it affects the law
                        that governs succession. Since 17.8.2016 a new European Regulation affects succession within EU
                        member states. The UK and Eire have not opted in to the Regulation (Denmark has opted out) but
                        it will nonetheless affect individuals connected with the EU. If you are a beneficiary of this
                        estate and there are non-UK assets involved, you should consider your own Will now. Do not wait
                        until the estate has been administered.</p>
                </b-collapse>
            </yes-no>
        </content-box>
