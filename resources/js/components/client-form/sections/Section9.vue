<template>
    <div class="container">
        <content-box title="Section 8 - Nil Rate Band Discretionary Trusts">
            <p class="text-gray-500">The nil rate band is also called the Inheritance tax threshold. It represents the
                first part of the estate which is taxed at 0%. On 5.4.2016 this threshold was £325,000.</p>
            <p class="text-gray-500">
                If the deceased was a widow or widower, you have been asked in section to give details of the estate of
                the first spouse to die. This is necessary to claim both the deceased’s nil-rate band plus the nil rate
                band of the first spouse to die.<br />
                Therefore, up to £650,000 of the estate can pass free of tax, but this is not always so.<br />
                If the first spouse to die created a Nil Rate Band Discretionary Trust in their Will, the transferrable
                nil rate band may not be available.<br />
                If you have provided a copy of the Will of the first spouse to die in response to the questions in 3.3
                then details of any Trust will be seen.
            </p>
        </content-box>

        <content-box title="8.1 - Nil Rate Band Discretionary relative" class="relative"
            v-for="(trust, key) in formData" :key="key">
            <a class="remove-trustee" href="#" v-if="formData.length > 1" @click.prevent="removeTrustee(key)">Remove
                Trustee</a>
            <p class="text-gray-500">If you know details of any Nil Rate Band Discretionary Trust, then please provide
                the following information:-</p>
            <honorific v-model="trust.honorific" />
            <base-input label="Forename" placeholder="John" v-model="trust.forename"></base-input>
            ...
            <div class="form-group">
                ...
                <switch-with-text v-model="trust.query2.answer" left-text="The Trustee"
                    right-text="The trustee’s solicitors or accountants" return-text />
            </div>
            <b-collapse :visible="trust.query2.answer == 'The trustee’s solicitors or accountants'">
                ...
                <base-input label="Forename" placeholder="John" v-model="trust.query2.onTrue.forename"></base-input>
        ...
        <content-box title="8.1 - Nil Rate Band Discretionary Trusts" v-if="formData.length < 4">
            <div class="text-center">
                <base-button type="default" outline @click="addTrustee">Add trustee<i
                        class="fas fa-plus ml-3"></i></base-button>
            </div>
        </content-box>
