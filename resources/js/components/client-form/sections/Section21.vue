<template>
    <div class="container">
        <content-box title="Section 20 - Other information">
            <p class="text-gray-500 m-0">Use this section as an opportunity to provide any extra information you believe
                relevant.</p>
        </content-box>

        <content-box title="20.1 Other information" v-for="(row, i) in formData" :key="`otherInfo${i}`"
            class="relative">
            <a class="remove-info" href="#" v-if="formData.length > 1" @click.prevent="removeInfo(i)">Remove</a>
            <label>Subject</label>
            <base-input placeholder="e.g. Section 13 - policy information finished on it’s way"
                v-model="row.subject"></base-input>

            <label>Description</label>
            <textarea v-model="row.description" class="form-control form-group"
            ...
            <div class="row mb-2" v-for="(doc, j) in row.docs" :key="j">
                <div class="col-12 file-row">
                    <client-file-upload class="mb-0" v-model="formData[i].docs[j]"
            ...
            <div class="row">
                <div class="col-12">
                    <client-file-upload class="mb-0" v-model="formData[i].docs[formData[i].docs.length]"
                        @input="saveData" wipeAfterInput></client-file-upload>
                </div>
            </div>
        </content-box>

        <content-box class="p-0 text-center" :shadow="false" :whiteBg="false">
            <button class="btn btn-primary shadow" @click="addRow">Add information<i data-v-c216df9e=""
